...
try:  # Prefer stdlib zoneinfo when available
...
except ImportError:  # pytz is optional for the suite
    pytz = None
...
def convert_ist_to_utc(ist_time_str):
    ...
    try:
        ...
        if pytz is not None:  # legacy dependency path
            ist = pytz.timezone("Asia/Kolkata")
            local_time = ist.localize(local_time)
            utc_time = local_time.astimezone(pytz.utc)
        elif ZoneInfo is not None:
            ist = ZoneInfo("Asia/Kolkata")
            utc_time = local_time.replace(tzinfo=ist).astimezone(timezone.utc)
        else:
            # Manual offset fallback (+5:30)
            ist_offset = timezone(timedelta(hours=5, minutes=30))
            utc_time = local_time.replace(tzinfo=ist_offset).astimezone(timezone.utc)
